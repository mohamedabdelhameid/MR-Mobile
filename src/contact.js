...
const Contact = () => {
  ...
  return (
    <>
      <MyNavbar />
      <div style={{ marginTop: "85px", padding: "10px", direction: "rtl", maxWidth: "800px" }} className="container">
        <h2 className="mb-4">تواصل معنا</h2>
        {message && <div className={`alert ${message.includes("✅") ? "alert-success" : "alert-danger"}`}>{message}</div>}

        <p className="text-warning"> نشكرك على تواصلك، وسنقوم بالرد عليك عبر البريد الإلكتروني في غضون ٤٨ ساعة  </p>
        ...
        <div className="mt-4">
          ...
          <p><FaMapMarkerAlt /> شارع أحمد بدوي، المراغة</p>
          <p><FaMapMarkerAlt /> شارع عبد المنعم رياض، المراغة</p>
        </div>
      </div>
      <Footer />
    </>
